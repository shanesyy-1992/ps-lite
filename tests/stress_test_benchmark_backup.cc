...
void RunWorker(int argc, char *argv[], KVWorker<char>* kv, int tid, int nthread) {
  ...
  std::vector<SArray<char> > server_vals;
  std::vector<std::vector<SArray<Key> > > server_keys;
  std::vector<SArray<int> > server_lens;
  for (int key = 0; key < total_key_num; key++) {
    void* ptr;
  ...
  for (int key = 0; key < total_key_num; key++) {
    ...
    std::vector<SArray<Key> > server_keys_for_threads;
    for (int thread_id = 0; thread_id < nthread; thread_id ++) {
      ...
      ps::Key ps_key = krs[server].begin() + key * nthread + thread_id;
      memcpy(ptr_key, &ps_key, sizeof(Key));
      server_keys_for_threads.push_back(keys);
      ...
      // Only let one thread do init, concurrent use of the same key with
      // RDMA van will cause hanging issue.
      if (thread_id == tid)
        kv->Wait(kv->ZPush(keys, vals, lens));
    }
    server_keys.push_back(server_keys_for_threads);
  }
    ...
    case PUSH_THEN_PULL: {
      ...
      for (int i = 0; i < repeat; ++i) {
        ...
        for (int server = 0; server < num_servers; server++) {
          auto keys = server_keys[server][tid];
